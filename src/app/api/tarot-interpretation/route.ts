...
export async function POST(request: NextRequest) {
  try {
    ...
    const interpretations = cards.map((card) => {
      const categoryKey = category as keyof typeof card.upright;
      const categoryInterpretation = card.upright[categoryKey];
      return {
        cardId: card.id,
